// __html__ is automatically provided by Figma from ui.html in manifest.json
// No need to declare or assign it - Figma handles it automatically
...
async function loadFonts(): Promise<void> {
  try {
    ...
    // If fonts fail to load, try just Regular
    await figma.loadFontAsync({ family: 'Inter', style: 'Regular' });
  }
}

// Create a text node
async function createTextNode(text: string, x: number, y: number, fontSize: number = 12, fontStyle: 'Regular' | 'Medium' | 'Bold' = 'Regular'): Promise<TextNode> {
  // Load the specific font we need BEFORE creating the text node
  try {
    ...
    try {
      ...
      // If Inter doesn't work, try Roboto as fallback
...
async function importJsonData(data: any) {
  try {
    ...
    if (data.userJourneys) {
      y += sectionSpacing;
      const journeyHeader = await createTextNode('User Journeys', startX, y, 18, 'Bold');
      y += 60;

      for (const journey of data.userJourneys) {
        const journeyTitle = await createTextNode(journey.name, startX, y, 14, 'Medium');
        y += 40;

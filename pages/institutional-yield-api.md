...
**One team operates the system you embed.** Kamino runs the oracle pricing on [Scope](./scope.md), the risk management on the [Vault engine](./vault-engine.md), and the white-glove integration end to end, with operational support and defined redemption SLAs. The operating model is explicit: the pricing, the rebalancing, the redemptions, and the monthly attestations are each run by a named team with direct lines to risk and engineering, so every moving part of the product you embed has a clear operator behind it.

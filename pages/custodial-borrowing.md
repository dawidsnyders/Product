...
One party runs all of it end to end. Kamino prices the collateral on [Scope](./scope.md), manages the risk and the default path through its own [liquidation engine](./liquidation-engine.md), gates access through [Permissioning](./permissioning.md), and stands up the market in a white-glove onboarding it operates with the borrower — with named contacts, defined SLAs on monitoring and liquidation, and a counterparty an institution can hold answerable for how the facility behaves. Custody at a qualified bank, continuous attested pricing, a tested liquidation path, compliance gating, and a configurable market combine into a borrowing structure a regulated institution can put balance-sheet capital behind, backed by an operator that answers for the whole stack.
